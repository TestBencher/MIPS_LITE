...
void execute_instruction(uint32_t instruction) {
    ...
    total_instructions++;
    if (opcode <= 0x0A) {
        if (opcode <= 0x05) arithmetic_count++;
        else logical_count++;
    } else if (opcode == 0x0C || opcode == 0x0D) {
        memory_count++;
    ...
    switch (opcode) {
        case 0x00: registers[rd] = registers[rs] + registers[rt]; PC += 4; break;
        case 0x01: registers[rt] = registers[rs] + imm; PC += 4; break;
        case 0x02: registers[rd] = registers[rs] - registers[rt]; PC += 4; break;
        case 0x03: registers[rt] = registers[rs] - imm; PC += 4; break;
        case 0x04: registers[rd] = registers[rs] * registers[rt]; PC += 4; break;
        case 0x05: registers[rt] = registers[rs] * imm; PC += 4; break;
        case 0x06: registers[rd] = registers[rs] | registers[rt]; PC += 4; break;
        case 0x07: registers[rt] = registers[rs] | imm; PC += 4; break;
        case 0x08: registers[rd] = registers[rs] & registers[rt]; PC += 4; break;
        case 0x09: registers[rt] = registers[rs] & imm; PC += 4; break;
        case 0x0A: registers[rd] = registers[rs] ^ registers[rt]; PC += 4; break;
        case 0x0B: registers[rt] = registers[rs] ^ imm; PC += 4; break;
        case 0x0C: registers[rt] = memory[(registers[rs] + imm) / 4]; PC += 4; break;
        case 0x0D: memory[(registers[rs] + imm) / 4] = registers[rt]; PC += 4; break;
        case 0x0E: PC += (registers[rs] == 0) ? imm * 4 : 4; break;
        case 0x0F: PC += (registers[rs] == registers[rt]) ? imm * 4 : 4; break;
        case 0x10: PC = registers[rs]; break;
        case 0x11:
